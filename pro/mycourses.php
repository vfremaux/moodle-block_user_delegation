...
$PAGE->requires->js('/blocks/user_delegation/pro/js/mycourses.php?id='.$courseid);

$url = new moodle_url('/blocks/user_delegation/pro/mycourses.php', array('id' => $blockid, 'course' => $courseid));
$PAGE->set_url($url);
...
$perpage = 10;
$usercourses = userdelegation::get_user_courses_bycap($USER->id, 'block/user_delegation:owncourse', false);
$coursescount = count($usercourses);
...
if (!empty($usercourses)) {
    ...
    $pageurl = new moodle_url('/blocks/user_delegation/pro/mycourses.php');
    echo $OUTPUT->paging_bar($coursescount, $page, $perpage, $pageurl);
} else {
    echo '<br/>';
