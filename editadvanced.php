<?php // $Id: editadvanced.php,v 1.4 2012-03-01 23:39:22 wa Exp $

    require_once('../../config.php');
    require_once($CFG->libdir.'/gdlib.php');
    require_once($CFG->libdir.'/adminlib.php');
    require_once($CFG->dirroot.'/blocks/userdelegation/editadvanced_form.php');
    ...
    print('<script type="text/javascript" src="js/jquery-1.4.2.min.js" ></script>');
     echo('<script type="text/javascript" src="js/user_edit.php?id='.$course.'" ></script>');
    ...
    if ($id == -1) {
        // creating new user
        require_capability('block/userdelegation:cancreateusers', $coursecontext);
        $user = new object();
        $user->id = -1;
        $user->auth = 'manual';
        $user->confirmed = 1;
        ...
        require_capability('block/userdelegation:isbehalfof', $personalcontext);    
    ...
    if ($usernew = $userform->get_data()) {
        ...
        if ($usernew->id == -1) {
            ...
            $personalcontext = get_context_instance(CONTEXT_USER,$usernew->id);
           //  DebugBreak();
        ...
        if ($user->id == $USER->id) {
            ...
            redirect("$CFG->wwwroot/blocks/userdelegation/user.php?course=".$course->id);
        }
        //never reached
    }
