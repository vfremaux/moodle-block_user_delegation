<?php // $Id: user.php,v 1.2 2012-12-11 17:24:17 vf Exp $
    ...
    $site = get_site();
	$courseparam = ($courseid > SITEID) ? "course={$course->id}&amp;" : '' ;
    
    //simply test .
    $someusercontext = get_context_instance(CONTEXT_USER,6);
    if(has_capability('block/userdelegation:isbehalfof',$someusercontext)){
	     //   print("Great ! its working");
	     //   print_object($someusercontext);
    }
    ...
    print_header_simple($SITE->shortname,$SITE->shortname);
    ...
    if ($confirmuser and confirm_sesskey()) {
       ...
       // }
        ...
        if ($confirm != md5($delete)) {
        	...
        	// unbinds user from me
        	/**
        	$personalcontext = get_context_instance(CONTEXT_USER,$user->id);
        	$role = get_record('role','shortname',$CFG->block_userdelegation_co_role);
        
            if($role){
                role_unassign($role->id, $USER->id, 0, $personalcontext->id);
            }
            */
            userdelegation::unattach_user($USER->id, $user->id);
    		...
    		if ($courses = get_user_courses_bycap($USER->id, 'block/userdelegation:cancreateusers', $USER->access, false)){
    			foreach($courses as $c){
    				$c_context = get_context_instance(CONTEXT_COURSE, $c->id);
			...
			if (!userdelegation::has_owners($user->id) && @$CFG->block_userdelegation_last_owner_deletes){
	            if (delete_user($user)) {
    ...
    foreach ($columns as $column) {
        ...
        $$column = "<a href=\"user.php?{$courseparam}sort=$column&amp;dir=$columndir\">".$string[$column]."</a>$columnicon";
    }
    ...
    $users = userdelegation::get_delegated_users($USER->id, $sort, $dir, $page * $perpage, $perpage, '', '', '', $extrasql);
    ...
    print_paging_bar($usercount, $page, $perpage, "user.php?{$courseparam}sort=$sort&amp;dir=$dir&amp;perpage=$perpage&amp;");
    ...
    if (!$users) {
        ...
        foreach ($users as $user) {
            ...
            if ($user->id == $USER->id) {
                ...
                if (has_capability('moodle/user:delete', $sitecontext)) {
                    $deletebutton = "<a href=\"user.php?{$courseparam}delete=$user->id&amp;sesskey=$USER->sesskey\">$strdelete</a>";
                } else {
                    $deletebutton ="";
                }
            }
            ...
            $isbehalfof = has_capability('block/userdelegation:isbehalfof', $usercontext);
            
            if ($isbehalfof and ($user->id != $mainadmin->id) and !is_mnet_remote_user($user)) {
                $editbutton = "<a href=\"$securewwwroot/blocks/userdelegation/editadvanced.php?{$courseparam}id=$user->id\">$stredit</a>";
                if ($user->confirmed == 0) {
                    $confirmbutton = "<a href=\"user.php?{$courseparam}confirmuser=$user->id&amp;sesskey=$USER->sesskey\">" . get_string('confirm') . "</a>";
                } else {
                    $confirmbutton = "";
                }
            } else {
            ...
            $table->data[] = array ("<a href=\"../../user/view.php?{$courseparam}id=$user->id\">$fullname</a>",
                                "$user->email",
                                "$user->city",
                                "$user->country",
                                $strlastaccess,
    ...
    if (has_capability('block/userdelegation:cancreateusers', $coursecontext)) {
        print_heading('<a href="'.$securewwwroot.'/blocks/userdelegation/editadvanced.php?'.$courseparam.'id=-1">'.get_string('addnewuser').'</a>');
     //   print_heading('<a href="'.$securewwwroot.'/blocks/userdelegation/uploaduser.php?{$courseparam}">'.get_string('uploaduser').'</a>');     
    }
    
      print('<div class="userpage-toolbar">
      <img src="images/folders.png" /> <a href="'.$securewwwroot.'/blocks/userdelegation/mycourses.php?'.$courseparam.'">'.get_string('mycourses').'</a>
      | '); 
     
    if (has_capability('block/userdelegation:cancreateusers', $coursecontext)) { 
      	print('<img src="images/upload.png" /><a href="'.$securewwwroot.'/blocks/userdelegation/uploaduser.php?'.$courseparam.'">'.get_string('uploadusers').'</a>      ');
	}
    ...
    if (!empty($table)) {
		...
		print('<div class="myuserstable"> ');
        print_table($table);
        print('</div>');
        print_paging_bar($usercount, $page, $perpage,
                         "user.php?{$courseparam}sort=$sort&amp;dir=$dir&amp;perpage=$perpage&amp;");
    ...
    if (!$users) {  
    	$noownedusersstr = get_string('noownedusers', 'block_userdelegation');
		print("<div style='font-weight:bold;margin-top:50px;text-align:center;'>$noownedusersstr</div>");   
    }
    
    if ($courseid == SITEID){
    	echo '<center><br/>';
    	print_single_button($CFG->wwwroot, array(), get_string('backtohome', 'block_userdelegation'));
    	echo '<br/><center>';
    } else {
    	echo '<center><br/>';
    	print_single_button($CFG->wwwroot.'/course/view.php', array('id' => $course->id), get_string('backtocourse', 'block_userdelegation'));
    	echo '<br/><center>';
    }
